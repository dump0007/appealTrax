...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        ...
        setLoading(true)
        const [firData, dashboardData, cityData, motionData, affidavitData, writTypeData] = await Promise.all([
          fetchFIRs(),
          fetchFIRDashboard(),
          fetchFIRCityGraph(),
          fetchMotionDashboard(),
          fetchAffidavitDashboard(),
          fetchWritTypeDistribution(),
        ])
        if (!active) {
          return
        }
  ...
  useEffect(() => {
    if (firs.length === 0) {
      // No FIRs – clear events and ensure loading state is reset
    ...
    async function loadEvents() {
      try {
        ...
        console.error('Failed to load proceedings events:', err)
        if (active) {
          setProceedingEvents([])
        }
      } finally {
